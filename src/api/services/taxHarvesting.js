...
/** Mock capital-gains API (promise + delay). */
export function mockFetchCapitalGains() {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(normalizeCapitalGainsResponse(MOCK_CAPITAL_GAINS_RESPONSE));
    }, 450);
  });
}

/** Mock holdings API — slower so the grid loading overlay is visible. */
export function mockFetchHoldings() {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(normalizeHoldingsResponse(MOCK_HOLDINGS_RAW));
    }, 1100);
  });
}

/** Raw Capital Gains API shape */
const MOCK_CAPITAL_GAINS_RESPONSE = {
  capitalGains: {
    stcg: {
      profits: 70200.88,
      losses: 1548.53,
...
/** Raw Holdings API response (totalHolding, same field names as backend) */
const MOCK_HOLDINGS_RAW = [
  {
    coin: "USDC",
    coinName: "USDC",
...
/**
 * UI + harvest math shape (matches prior app expectations).
 * `losses` are positive magnitudes; nets = profits - losses.
 */
export function normalizeCapitalGainsResponse(api) {
  const { stcg, ltcg } = api.capitalGains;
  ...
  return {
    ...
    realisedCapitalGains: stNet + ltNet,
  };
}

/** Stable row id for AG Grid when `coin` repeats (e.g. two USDC rows). */
export function normalizeHoldingsResponse(rows) {
  return rows.map((row, index) => ({
    ...row,
    rowKey: `${row.coin}-${index}`,
    totalHoldings: row.totalHolding,
  }));
}

export const TAX_HARVESTING_API_KEY = "taxHarvestingApi";

export const taxHarvestingApi = createApi({
  reducerPath: TAX_HARVESTING_API_KEY,
  baseQuery: fakeBaseQuery(),
  endpoints: (builder) => ({
    getCapitalGains: builder.query({
      async queryFn() {
        const data = await mockFetchCapitalGains();
        return { data };
      },
    }),
    getHoldings: builder.query({
      async queryFn() {
        const data = await mockFetchHoldings();
        return { data };
      },
    }),
  }),
...
export const { useGetCapitalGainsQuery, useGetHoldingsQuery } =
  taxHarvestingApi;
